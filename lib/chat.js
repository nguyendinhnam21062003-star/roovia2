...
function buildReplyModel({ mode = 'consultation', roomName = '', checkIn = '', checkOut = '', guests = '' }) {
  ...
  const intro = `Xin chào quý khách, chúng tôi đã nhận được yêu cầu ${requestLabel}${subjectText}.`;
  const highlights = [
    ...
    guests ? { label: 'Số khách', value: formatGuestCount(guests) } : null
  ].filter(Boolean);
  ...
  return {
    ...
    subject,
    highlights,
    note: 'Để lại số điện thoại hoặc Zalo, nhân viên kinh doanh sẽ liên hệ lại để xác nhận.',
    emergency: 'Cần hỗ trợ gấp? Gọi 0328 495 797 hoặc chọn Zalo/Fanpage bên dưới.',
    contacts: [
      { label: 'Gọi ngay', href: 'tel:0328495797', icon: 'phone', kind: 'link', tone: 'call' },
      { label: 'Zalo', icon: 'message-circle', kind: 'tag', tone: 'soft' },
      { label: 'Fanpage', icon: 'facebook', kind: 'tag', tone: 'soft' }
    ]
  };
}
